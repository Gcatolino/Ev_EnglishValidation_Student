      ...
      if (flag == 1) { // Preleva tutte le richieste
        try {
          ...
          sql = "SELECT r.id_request, r.certificate_serial, r.level, r.release_date,"
              ...
              + ")";
          ResultSet r = stmtSelect.executeQuery(sql); //il parametro passato a executeQuery era null
          if (r.wasNull()) {
    	 ...
    	 /* if(flag == 2)
    		  throw new IllegalArgumentException("Parametro non valido"); lancia un'eccezione inutile*/
        ...
        try {
          ...
          stmt = conn.prepareStatement(sql);
          stmt.setInt(2, idRequest); //parametri invertiti
